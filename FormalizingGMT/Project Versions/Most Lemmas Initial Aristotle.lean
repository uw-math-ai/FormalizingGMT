...
/-
Definition of s-density ratio and s-density. Note: The user text says "D^s(E,x) exists", which implies a limit. I defined Ds as liminf to have a value always, but HasDs checks if it is a limit.
Actually, looking at the text "D^s(E,x) fails to exist", it implies we should distinguish between existence and the value.
...
If it doesn't exist, `Ds` (liminf) is still defined but might not be the limit (limit might not exist).
However, usually "density exists" means limsup = liminf.
I'll define `Ds` as `liminf` (or `limsup`? The text uses `limsup` in Lemma 2.4 "upper density lower bound").
...
noncomputable def densityRatio (s : ℝ) (E : Set (EuclideanSpace ℝ (Fin n))) (x : EuclideanSpace ℝ (Fin n)) (r : ℝ) : ℝ≥0∞ :=
  hausdorffMeasure s (E ∩ ball x r) / (ENNReal.ofReal ((2 * r) ^ s))

/-- The s-density of E at x. -/
noncomputable def Ds (s : ℝ) (E : Set (EuclideanSpace ℝ (Fin n))) (x : EuclideanSpace ℝ (Fin n)) : ℝ≥0∞ :=
...
theorem diam_ball_eq_two_mul_radius (hn : n ≠ 0) (x : EuclideanSpace ℝ (Fin n)) (r : ℝ) (hr : 0 < r) :
  EMetric.diam (ball x r) = ENNReal.ofReal (2 * r) := by
    refine' le_antisymm _ _
    · -- For any two points $y, z \in \text{ball}(x, r)$, we have $\text{dist}(y, z) \leq 2r$.
      have h_dist_le : ∀ y z : EuclideanSpace ℝ (Fin n), y ∈ Metric.ball x r → z ∈ Metric.ball x r → dist y z ≤ 2 * r := by
        exact fun y z hy hz => le_trans ( dist_triangle_right _ _ _ ) ( by linarith [ Metric.mem_ball.mp hy, Metric.mem_ball.mp hz ] );
      refine' EMetric.diam_le _;
      exact fun y hy z hz => by simpa only [ edist_dist ] using ENNReal.ofReal_le_ofReal ( h_dist_le y z hy hz ) ;
    · -- For any ε > 0, there exist points y and z in the ball such that dist y z > 2r - ε.
      have h_eps : ∀ ε > 0, ∃ y z : EuclideanSpace ℝ (Fin n), y ∈ Metric.ball x r ∧ z ∈ Metric.ball x r ∧ dist y z > 2 * r - ε := by
        intro ε ε_pos
        obtain ⟨δ, hδ_pos, hδ_lt⟩ : ∃ δ > 0, δ < r ∧ ε > 2 * δ := by
          exact ⟨ Min.min ( ε / 4 ) ( r / 2 ), lt_min ( by positivity ) ( by positivity ), min_lt_of_right_lt ( by linarith ), by linarith [ min_le_left ( ε / 4 ) ( r / 2 ), min_le_right ( ε / 4 ) ( r / 2 ) ] ⟩;
        -- Choose points $y$ and $z$ in the ball such that $dist y z = 2r - 2\delta$.
        obtain ⟨y, z, hyz⟩ : ∃ y z : EuclideanSpace ℝ (Fin n), y ∈ Metric.ball x r ∧ z ∈ Metric.ball x r ∧ dist y z = 2 * r - 2 * δ := by
          -- Choose points $y$ and $z$ in the ball such that $dist y z = 2r - 2\delta$. We can take $y = x + (r - \delta) \cdot e_1$ and $z = x - (r - \delta) \cdot e_1$, where $e_1$ is the first standard basis vector.
          use x + (r - δ) • (EuclideanSpace.single ⟨0, Nat.pos_of_ne_zero hn⟩ 1), x - (r - δ) • (EuclideanSpace.single ⟨0, Nat.pos_of_ne_zero hn⟩ 1);
          norm_num [ dist_eq_norm, EuclideanSpace.norm_eq ];
          exact ⟨ by rw [ Real.sqrt_sq ] <;> linarith, by rw [ Real.sqrt_eq_iff_mul_self_eq ] <;> norm_num [ Finset.sum_add_distrib, add_sq, Finset.mul_sum _ _ _, Finset.sum_mul _ _ _, Finset.sum_ite, Finset.filter_eq', Finset.filter_ne' ] <;> nlinarith ⟩;
      ...
      simp_all only [ne_eq, Nat.ofNat_nonneg, ofReal_mul, ofReal_ofNat, gt_iff_lt, sub_pos, mem_ball,
        _root_.sub_sub_cancel]
      apply And.intro
      ...
      · intro x_1 x_2 _ __1
        · refine' le_trans _ ( ENNReal.toReal_mono _ <| EMetric.edist_le_diam_of_mem ( Metric.mem_ball.mpr ‹Dist.dist x_1 x < r› ) ( Metric.mem_ball.mpr ‹Dist.dist x_2 x < r› ) );
          · norm_num [ edist_dist ];
          simp_all only [ne_eq]
...
noncomputable def hausdorffContent (s : ℝ) (E : Set (EuclideanSpace ℝ (Fin n))) : ℝ≥0∞ :=
  ⨅ (t : ℕ → Set (EuclideanSpace ℝ (Fin n))) (h_cov : E ⊆ ⋃ i, t i), ∑' i, (EMetric.diam (t i)) ^ s

theorem hausdorffContent_le_hausdorffMeasure (s : ℝ) (E : Set (EuclideanSpace ℝ (Fin n))) (hs : 0 < s) :
  hausdorffContent s E ≤ hausdorffMeasure s E := by
    ...
    -- For any $r > 0$, the infimum of the sums over covers with diameters $\leq r$ is at least the Hausdorff content.
    have h_inf_ge_hausdorff_content : ∀ r > 0, ⨅ (t : ℕ → Set (EuclideanSpace ℝ (Fin n))), ⨅ (_ : E ⊆ Set.iUnion t), ⨅ (_ : ∀ n, EMetric.diam (t n) ≤ r), ∑' n, ⨆ (_ : (t n).Nonempty), EMetric.diam (t n) ^ s ≥ hausdorffContent s E := by
      intro r hr; refine' le_iInf fun t => le_iInf fun ht => le_iInf fun ht' => _;
      ...
      by_cases hi : ( t i ).Nonempty <;> aesop;
...
/-
Lemma 2.3: Approximation by compact sets. If E is an s-set, there exists a compact subset F with positive Hausdorff measure.
-/
open MeasureTheory MeasureTheory.Measure Metric Set Filter Topology ENNReal

variable {n : ℕ}

lemma exists_compact_subset_pos_measure (s : ℝ) (E : Set (EuclideanSpace ℝ (Fin n))) (hE : IsSSet s E) :
...
noncomputable def upperDs (s : ℝ) (E : Set (EuclideanSpace ℝ (Fin n))) (x : EuclideanSpace ℝ (Fin n)) : ℝ≥0∞ :=
...
lemma exists_accPt_of_pos_hausdorffMeasure (s : ℝ) (E : Set (EuclideanSpace ℝ (Fin n))) (hs : 0 < s) (hE : 0 < hausdorffMeasure s E) :
  ∃ x, AccPt x (𝓟 E) := by
    by_contra! h;
    ...
    have h_discrete : DiscreteTopology E := by
      ...
      have h_isolated : ∀ x ∈ E, ∃ U : Set (EuclideanSpace ℝ (Fin n)), IsOpen U ∧ x ∈ U ∧ U ∩ E = {x} := by
        ...
        obtain ⟨w, h⟩ := h
        obtain ⟨left, right⟩ := h
        exact ⟨ Metric.ball x w, Metric.isOpen_ball, Metric.mem_ball_self left, Set.eq_singleton_iff_unique_mem.mpr ⟨ ⟨ Metric.mem_ball_self left, hx ⟩, fun y hy => Classical.not_not.1 fun h => right hy.1 ( by simp_all only [dist_self,
          mem_inter_iff, mem_ball, not_false_eq_true] ) hy.2 ⟩ ⟩
    ...
    have h_hausdorff_zero : ∀ x ∈ E, μH[s] {x} = 0 := by
      ...
      · refine' iSup_le fun r => iSup_le fun hr => _;
        ...
        simp_all only [iUnion_singleton_eq_range, range_const, subset_refl, EMetric.diam_singleton, zero_le,
          implies_true, singleton_nonempty, zero_rpow_of_pos, iSup_pos, tsum_zero, iInf_pos, le_refl]
      · exact zero_le _;
    convert MeasureTheory.measure_iUnion_null fun x : E => h_hausdorff_zero x x.2;
    simp_all only [countable_coe_iff, iUnion_singleton_eq_range, Subtype.range_coe_subtype, setOf_mem_eq, false_iff]
    apply Aesop.BuiltinRules.not_intro
    intro a
    simp_all only [lt_self_iff_false]

/-
Lemma 2.6: Ball contained in annulus. If x != y, r = |x-y|, 0 < eta < 1, then B_{1/2 r eta}(x) is contained in the annulus A_{r,eta} centered at y.
-/
open MeasureTheory MeasureTheory.Measure Metric Set Filter Topology ENNReal

variable {n : ℕ}

lemma ball_contained_in_annulus (x y : EuclideanSpace ℝ (Fin n)) (hxy : x ≠ y) (η : ℝ) (hη : 0 < η) (hη1 : η < 1) :
  let r := dist x y
...
lemma annular_density_ratio_lower_bound (s : ℝ) (E : Set (EuclideanSpace ℝ (Fin n))) (hs : 0 < s) (hs1 : s < 1)
  (F : Set (EuclideanSpace ℝ (Fin n))) (hF_subset : F ⊆ E) (hF_sset : IsSSet s F)
  (ρ : ℝ) (hρ : 0 < ρ)
  (h_density : ∀ x ∈ F, ∀ r, 0 < r → r ≤ ρ → hausdorffMeasure s (E ∩ ball x r) > ENNReal.ofReal ((1/2) * (2 * r) ^ s))
  (x y : EuclideanSpace ℝ (Fin n)) (hx : x ∈ F) (hy : y ∈ F) (hxy : x ≠ y)
  (η : ℝ) (hη : 0 < η) (hη1 : η < 1)
  ...
  ENNReal.ofReal ((1/2) * r ^ s * η ^ s) < hausdorffMeasure s (E ∩ A) := by
    ...
    have h_ball_subset_annulus : Metric.ball x (1 / 2 * r * η) ⊆ Metric.ball y (r * (1 + η)) \ Metric.ball y (r * (1 - η)) := by
      intro z hz;
      subst hr_def
      simp_all only [one_div, inv_nonneg, Nat.ofNat_nonneg, ofReal_mul, Nat.ofNat_pos, ofReal_inv_of_pos,
        ofReal_ofNat, gt_iff_lt, ne_eq, mem_ball, mem_diff, not_lt]
      apply And.intro
      · have := dist_triangle z x y;
        norm_num at * ; nlinarith [ dist_pos.mpr hxy ];
      · have := dist_triangle_left x y z ; norm_num at * ; nlinarith [ @dist_nonneg _ _ x y, @dist_nonneg _ _ z y ];
    -- By Lemma 2.6, we have $E \cap B_{1/2 r η}(x) \subset E \cap A_{r,η}$.
    have h_inter_subset_annulus : E ∩ Metric.ball x (1 / 2 * r * η) ⊆ E ∩ (Metric.ball y (r * (1 + η)) \ Metric.ball y (r * (1 - η))) := by
      exact Set.inter_subset_inter_right _ h_ball_subset_annulus;
    -- By Lemma 2.6, we have $H^s(E \cap B_{1/2 r η}(x)) > 1/2 (2r')^s$.
    have h_measure_ball : μH[s] (E ∩ Metric.ball x (1 / 2 * r * η)) > ENNReal.ofReal (1 / 2 * (2 * (1 / 2 * r * η)) ^ s) := by
      exact h_density x hx _ ( mul_pos ( mul_pos ( by norm_num ) ( hr_def.symm ▸ dist_pos.mpr hxy ) ) hη ) ( by rw [ le_div_iff₀ hη ] at hr_bound; nlinarith );
    convert lt_of_lt_of_le h_measure_ball ( MeasureTheory.measure_mono h_inter_subset_annulus ) using 1 ; ring_nf;
...
lemma limit_density_ratio_scaled (s : ℝ) (E : Set (EuclideanSpace ℝ (Fin n))) (y : EuclideanSpace ℝ (Fin n))
  ...
  Tendsto (fun r => hausdorffMeasure s (E ∩ ball y (r * c)) / ENNReal.ofReal ((2 * r) ^ s)) (𝓝[>] 0)
    (𝓝 (Ds s E y * ENNReal.ofReal (c ^ s))) := by
      ...
      have h_scaled : ∀ r > 0, (hausdorffMeasure s (E ∩ ball y (r * c))) / (ENNReal.ofReal ((2 * r) ^ s)) = (hausdorffMeasure s (E ∩ ball y (r * c))) / (ENNReal.ofReal ((2 * r * c) ^ s)) * ENNReal.ofReal (c ^ s) := by
        -- By properties of exponents, we can rewrite the denominator as $(2r)^s \cdot c^s$.
        intro r hr
        have h_denom : ENNReal.ofReal ((2 * r * c) ^ s) = ENNReal.ofReal ((2 * r) ^ s) * ENNReal.ofReal (c ^ s) := by
          rw [ ← ENNReal.ofReal_mul ( by positivity ), Real.mul_rpow ( by positivity ) ( by positivity ) ];
        rw [ ENNReal.div_mul ] ; ring_nf at * ;
        simp_all only [gt_iff_lt]
        · rw [ ENNReal.mul_div_cancel_right ( by positivity ) ( by simp_all only [ne_eq, ofReal_ne_top,
          not_false_eq_true] ) ];
      ...
      have h_original : Filter.Tendsto (fun r => (hausdorffMeasure s (E ∩ ball y (r * c))) / (ENNReal.ofReal ((2 * r * c) ^ s))) (𝓝[>] 0) (𝓝 (Ds s E y)) := by
        obtain ⟨ y, hy, hy' ⟩ := h_density;
        convert hy.comp ( show Filter.Tendsto ( fun r : ℝ => r * c ) ( 𝓝[>] 0 ) ( 𝓝[>] 0 ) from ?_ ) using 2;
        · unfold densityRatio;
          subst hy'
      ...
      have h_final : Filter.Tendsto (fun r => (hausdorffMeasure s (E ∩ ball y (r * c))) / (ENNReal.ofReal ((2 * r * c) ^ s)) * ENNReal.ofReal (c ^ s)) (𝓝[>] 0) (𝓝 (Ds s E y * ENNReal.ofReal (c ^ s))) := by
        -- Apply the fact that multiplication by a constant preserves limits in the topology of ENNReal.
        apply ENNReal.Tendsto.mul_const h_original;
        exact Or.inr ENNReal.ofReal_ne_top;
      exact h_final.congr' ( Filter.eventuallyEq_of_mem self_mem_nhdsWithin fun x hx => h_scaled x hx ▸ rfl )
...
lemma measure_annulus_eq_sub (s : ℝ) (E : Set (EuclideanSpace ℝ (Fin n))) (y : EuclideanSpace ℝ (Fin n))
  (hE : IsSSet s E) (r η : ℝ) (hr : 0 < r) (hη : 0 < η) :
  hausdorffMeasure s (E ∩ (ball y (r * (1 + η)) \ ball y (r * (1 - η)))) =
  hausdorffMeasure s (E ∩ ball y (r * (1 + η))) - hausdorffMeasure s (E ∩ ball y (r * (1 - η))) := by
    rw [ ← MeasureTheory.measure_diff ];
    apply congr_arg _
    ext
    simp_all only [mem_inter_iff, mem_diff, mem_ball, not_lt, not_and]
    apply Iff.intro
    · intro a
      simp_all only [and_self, imp_self]
    · intro a
      simp_all only [and_self]
    · exact Set.inter_subset_inter_right _ ( Metric.ball_subset_ball <| by nlinarith );
    · refine' hE.1.nullMeasurableSet.inter _;
...
lemma annular_density_ratio_limit (s : ℝ) (E : Set (EuclideanSpace ℝ (Fin n))) (y : EuclideanSpace ℝ (Fin n)) (hE : IsSSet s E)
  (h_density : HasDs s E y) (h_finite : Ds s E y ≠ ⊤)
  (η : ℝ) (hη : 0 < η) (hη1 : η < 1) :
  ...
  Tendsto (fun r => hausdorffMeasure s (E ∩ A r) / ENNReal.ofReal ((2 * r) ^ s)) (𝓝[>] 0)
    (𝓝 (Ds s E y * ENNReal.ofReal ((1 + η) ^ s - (1 - η) ^ s))) := by
      -- Using the fact that the measure of the annulus is the difference of the measures of the two balls, we can write the limit as the difference of the limits of the density ratios for each ball.
      have h_diff : Filter.Tendsto (fun r => (μH[s] (E ∩ ball y (r * (1 + η)))) / ENNReal.ofReal ((2 * r) ^ s) - (μH[s] (E ∩ ball y (r * (1 - η)))) / ENNReal.ofReal ((2 * r) ^ s)) (𝓝[>] 0) (𝓝 (Ds s E y * ENNReal.ofReal ((1 + η) ^ s) - Ds s E y * ENNReal.ofReal ((1 - η) ^ s))) := by
        -- Apply the fact that the difference of two functions that tend to their limits also tends to the difference of the limits.
        have h_diff : Filter.Tendsto (fun r => (μH[s] (E ∩ ball y (r * (1 + η)))) / ENNReal.ofReal ((2 * r) ^ s)) (𝓝[>] 0) (𝓝 (Ds s E y * ENNReal.ofReal ((1 + η) ^ s))) ∧ Filter.Tendsto (fun r => (μH[s] (E ∩ ball y (r * (1 - η)))) / ENNReal.ofReal ((2 * r) ^ s)) (𝓝[>] 0) (𝓝 (Ds s E y * ENNReal.ofReal ((1 - η) ^ s))) := by
          exact ⟨ by simpa using limit_density_ratio_scaled s E y h_density ( 1 + η ) ( by linarith ), by simpa using limit_density_ratio_scaled s E y h_density ( 1 - η ) ( by linarith ) ⟩;
      ...
      · filter_upwards [ self_mem_nhdsWithin ] with r hr;
        ...
        have h_annulus : μH[s] (E ∩ (Metric.ball y (r * (1 + η)) \ Metric.ball y (r * (1 - η)))) = μH[s] (E ∩ Metric.ball y (r * (1 + η))) - μH[s] (E ∩ Metric.ball y (r * (1 - η))) := by
          rw [ ← MeasureTheory.measure_diff ];
          refine congr_arg _ ?_
          ext
          simp_all only [ne_eq, mem_Ioi, mem_inter_iff, mem_diff, mem_ball, not_lt, not_and]
          apply Iff.intro
          · intro a
            simp_all only [and_self, imp_self]
          · intro a
            simp_all only [and_self]
          · exact Set.inter_subset_inter_right _ ( Metric.ball_subset_ball <| by nlinarith [ hr.out ] );
          · exact hE.1.nullMeasurableSet.inter ( measurableSet_ball.nullMeasurableSet );
          · exact ne_of_lt ( lt_of_le_of_lt ( MeasureTheory.measure_mono ( Set.inter_subset_left ) ) hE.2.2 );
        rw [ h_annulus, ENNReal.sub_div ];
        exact fun _ _ => ne_of_gt <| ENNReal.ofReal_pos.mpr <| Real.rpow_pos_of_pos ( mul_pos zero_lt_two hr ) _;
      · -- Since ENNReal.ofReal is linear, we can rewrite the right-hand side of the inequality.
        have h_linear : ENNReal.ofReal ((1 + η) ^ s - (1 - η) ^ s) = ENNReal.ofReal ((1 + η) ^ s) - ENNReal.ofReal ((1 - η) ^ s) := by
    ...
    have h_second_deriv : ∀ᶠ x in 𝓝 0, |(1 + x) ^ s - (1 - x) ^ s - 2 * s * x| ≤ 2 * |x|^2 := by
      ...
      have h_mvt : ∀ x ∈ Set.Ioo (-1 / 2) (1 / 2), x ≠ 0 → ∃ c ∈ Set.Ioo (min 0 x) (max 0 x), (1 + x) ^ s - (1 - x) ^ s - 2 * s * x = deriv (fun x => (1 + x) ^ s - (1 - x) ^ s - 2 * s * x) c * x := by
        ...
        have h_cont_diff : ContinuousOn (fun x => (1 + x) ^ s - (1 - x) ^ s - 2 * s * x) (Set.Icc (min 0 x) (max 0 x)) ∧ DifferentiableOn ℝ (fun x => (1 + x) ^ s - (1 - x) ^ s - 2 * s * x) (Set.Ioo (min 0 x) (max 0 x)) := by
          bound;
          · exact continuousOn_of_forall_continuousAt fun y hy => by exact ContinuousAt.sub ( ContinuousAt.sub ( ContinuousAt.rpow ( continuousAt_const.add continuousAt_id ) continuousAt_const <| Or.inr <| by linarith ) ( ContinuousAt.rpow ( continuousAt_const.sub continuousAt_id ) continuousAt_const <| Or.inr <| by linarith ) ) ( ContinuousAt.mul continuousAt_const continuousAt_id ) ;
      ...
      intro y a a_1
      by_cases hy : y = 0 <;> specialize h_mvt y <;> aesop;
      rcases h_mvt ( by norm_num at *; linarith [ abs_lt.mp a ] ) ( by norm_num at *; linarith [ abs_lt.mp a ] ) with ⟨ c, ⟨ hc₁, hc₂ ⟩, hc₃ ⟩ ; rw [ hc₃ ] ; rw [ abs_mul ]
      simp_all only [mul_eq_mul_right_iff, or_false]
      cases hc₁ with
      | inl h =>
        cases hc₂ with
        | inl h_1 => linarith
        | inr h_2 => exact le_trans ( mul_le_mul_of_nonneg_right ( H ( show |c| < ε by rw [ abs_of_pos h ] ; linarith [ abs_lt.mp a, abs_lt.mp a_1 ] ) ) ( abs_nonneg _ ) ) ( by rw [ abs_of_pos h ] ; rw [ abs_of_pos ( by linarith [ abs_lt.mp a, abs_lt.mp a_1 ] : 0 < y ) ] ; nlinarith [ abs_lt.mp a, abs_lt.mp a_1 ] );
      | inr h_1 =>
        cases hc₂ with
        | inl h => exact le_trans ( mul_le_mul_of_nonneg_right ( H ( show |c| < ε by exact abs_lt.mpr ⟨ by linarith [ abs_lt.mp a, abs_lt.mp a_1 ], by linarith [ abs_lt.mp a, abs_lt.mp a_1 ] ⟩ ) ) ( abs_nonneg _ ) ) ( by cases abs_cases y <;> cases abs_cases c <;> nlinarith );
        | inr h_2 => linarith;
